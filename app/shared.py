...
from mdn import MixtureDensityNetwork, fit_mdn
...
.lb-table th.num, .lb-table td.num {{
    text-align: right;
}}
.lb-table tbody td {{
...
.lb-row-best {{ background: {accent_bg}; }}
.lb-badge {{
    display: inline-block;
...
/* Must beat [stAppViewContainer] .stRadio div[role="radiogroup"] label = (0,3,0,2) */
...
def inject_global_styles():
    ...
    # Unescape {{ }} left from the original .format() convention
...
def metric_card(label, value, note=""):
    """Legacy alias for metric_tile()."""
    metric_tile(label, value, note)
...
def load_comparison_metrics() -> dict:
    """Build the comparison bundle, training any missing models first."""
    files = {
    ...
    if not files["linear"].exists():        fit_linear_regression()
    if not files["random_forest"].exists(): fit_random_forest()
    if not files["mlp"].exists():           fit_mlp()
    if not files["quantile"].exists():      fit_quantile_models()
    if not files["mdn"].exists():           fit_mdn()
    ...
    leaderboard = pd.DataFrame([
        {"Model": "Linear Regression", "R2": linear["r2_dollar"],         "RMSE": linear["rmse_dollar"],        "MAE": linear["mae_dollar"],          "Notes": "Baseline · log-charges target"},
        {"Model": "Random Forest",     "R2": rf["r2"],                    "RMSE": rf["rmse"],                   "MAE": rf["mae"],                     "Notes": "Tree ensemble · used in demo"},
        {"Model": "MLP",               "R2": mlp["r2"],                   "RMSE": mlp["rmse"],                  "MAE": mlp["mae"],                    "Notes": "Feed-forward neural network"},
        {"Model": "MDN",               "R2": mdn["r2"],                   "RMSE": mdn["rmse"],                  "MAE": mdn["mae"],                    "Notes": "From-scratch mixture density network"},
        {"Model": "Quantile Reg.",     "R2": qr["r2_median_dollar"],      "RMSE": qr["rmse_median_dollar"],     "MAE": qr["mae_median_dollar"],       "Notes": "Median + 80% interval"},
    ]).sort_values(["R2", "RMSE"], ascending=[False, True]).reset_index(drop=True)

    return {
        "leaderboard": leaderboard,
        "linear": linear, "random_forest": rf,
        "mlp": mlp, "quantile": qr, "mdn": mdn,
    }
...
def plot_model_comparison(leaderboard: pd.DataFrame):
    """Horizontal bar chart comparing model R² — cleaner read than vertical."""
